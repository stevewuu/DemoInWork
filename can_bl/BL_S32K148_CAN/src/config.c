...
{
//=============================== CAN PORT ====================================
    PCC->PCCn[BL_PCC_CAN] |= PCC_PCCn_CGC_MASK;   /* Enable clock for PORTE */
    PORT_CAN->PCR[0] |= PORT_PCR_MUX(5);          /* Port B: MUX = ALT5, CAN0_RX */
    PORT_CAN->PCR[1] |= PORT_PCR_MUX(5);          /* Port B: MUX = ALT5, CAN0_TX */
//=============================== LED PORT ====================================
    PCC->PCCn[BL_PCC_LED ]|=PCC_PCCn_CGC_MASK;    /* Enable clock for PORTD */
...
//=============================== BTN PORT ====================================
    PCC->PCCn[BL_PCC_BTN ]|=PCC_PCCn_CGC_MASK;    /* Enable clock for PORTD */
    PORT_BTN->PCR[BTN] =  PORT_PCR_MUX(1);        /* Port C13: MUX = GPIO (to btn0) */
    GPIO_BTN->PDDR &= ~(1<<BTN);                  /* Port C13: Data direction = output */
//=============================== 1169 SPI ====================================
#ifdef SBC_UJA1169
    /* If board has MC33904, SPI pin config. is required */
